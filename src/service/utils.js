...
export const assessTokenValidity = () => {
  ...
  if (tokenStillAlive(tokenDate, TOKEN_LIFE_DURATION)) {
    ...
    const refreshTokenIsValid = tokenStillAlive(refreshTokenDate, REFRESH_TOKEN_LIFE_DURATION)
    return refreshTokenIsValid ? handleRefreshTokenValid(status, refreshToken) : false
  }
}

const tokenStillAlive = (tokenDate, tokenLifeDuration) => Date.now() - tokenDate < tokenLifeDuration
